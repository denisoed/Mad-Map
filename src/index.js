...
class Map extends Component {
  ...
  handleTouchStart = (event) => {
    ...
    if (event.target && parentHasClass(event.target, 'mad-map-drag-block')) {
      return
    }
  ...
  handleMouseDown = (event) => {
    ...
    if (event.button === 0 &&
        (!event.target || !parentHasClass(event.target, 'mad-map-drag-block')) &&
  ...
  handleMouseUp = (event) => {
    ...
    if (this._mouseDown) {
      ...
      if (this.props.onClick &&
          (!event.target || !parentHasClass(event.target, 'mad-map-click-block')) &&
  ...
  renderAttribution () {
    ...
    return (
      <div key='attr' className='mad-map-attribution' style={style}>
        {attributionPrefix === false ? null : (
          <span>
            {attributionPrefix || <a href='https://github.com/denisoed/Mad-Map' target="_blank" style={linkStyle}>Mad Map</a>}
            {' | '}
          </span>
        )}
        {attribution || (<span>
  ...
  render () {
    ...
    return (
      <div
        ...
        ref={this.setRef}
        className={`mad-map-drag-block${className ? ` ${className}` : ''}`}>
        {this.props.children}
      </div>
    )
  }
